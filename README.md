...
It is accomplished by making use of EPT. EPT allows you to enforce a guest to
...
With this configuration, any read and write access triggers EPT violation
...
then interrupted by the VMM with MTF VM-exit.
 ...
 2. MTF VM-exit
After executing a single instruction, guest is interrupted by MTF VM-exit. on
...
disassembler. The following steps are how DdiMon monitors execution of 0xa234.
 ...
 1. #BP VM-exit
On #BP VM-exit, the VMM checks if guest IP is 0x1234 first. If so, next, it
...
breakpoint at a return address if a post handler is given.

After that, just like the case of the read and write access, the VMM changes an
EPT entry corresponds to 0x1000-0x1fff to refer to contents of 0xb000 and sets
MTF so that a guest can run an original instruction and be interrupted then.

 2. MTF VM-exit
On MTF VM-exit, the exact same operations are done as the case of the read and
write access.
